...
<section aria-labelledby="customer-card-title" class="space-y-5">
    ...
    @if ($available)
        <div data-customer-card-page class="mx-auto w-full max-w-[40rem]">
            <article id="customer-card-printable" data-customer-card-printable class="customer-card-grid relative isolate flex min-h-[15rem] flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-sm sm:aspect-[1.586/1] sm:min-h-0" aria-labelledby="customer-card-name">
                ...
                <div class="relative grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_minmax(5rem,auto)] items-start gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,10.5rem)] sm:gap-6 sm:p-6">
                    <div class="flex min-w-0 flex-col self-stretch">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                <p class="whitespace-nowrap text-[0.6875rem] font-bold uppercase tracking-[0.12em] text-forest-700 sm:text-caption"><span class="sm:hidden">Kartu nasabah</span><span class="hidden sm:inline">Kartu nasabah digital</span></p>
                                <p class="mt-0.5 hidden text-caption font-semibold text-text-secondary sm:block sm:text-body-sm">Bank Sampah Digital</p>
                            </div>
                            <span class="inline-flex shrink-0 items-center gap-1 rounded-full border border-forest-600/25 bg-success-bg px-1.5 py-0.5 text-[0.6875rem] font-bold text-forest-700 sm:px-2 sm:py-1 sm:text-caption">
                        ...
                        <div class="mt-auto min-w-0 pt-2 sm:pt-3">
                            <p class="text-[0.6875rem] font-semibold uppercase tracking-[0.1em] text-text-secondary sm:text-caption">Nama nasabah</p>
                            ...
                            <dl class="mt-2 grid gap-1 border-t border-border/80 pt-2 text-[0.6875rem] sm:mt-4 sm:grid-cols-2 sm:gap-x-5 sm:gap-y-2 sm:pt-3 sm:text-caption">
                                <div>
                                    <dt class="font-medium text-text-secondary">Nomor nasabah</dt>
                                    <dd data-customer-card-number class="amount-tabular mt-0.5 text-[0.75rem] font-bold text-deep-green sm:text-body-sm">{{ $customerNumber }}</dd>
                                </div>
                                <div>
                                    <dt class="font-medium text-text-secondary">Wilayah layanan</dt>
                                    <dd data-customer-card-area class="mt-0.5 truncate text-[0.75rem] font-bold text-deep-green sm:text-body-sm">{{ $serviceArea }}</dd>
                                </div>
                            </dl>
                        </div>

                        <p class="mt-3 hidden text-[0.6875rem] font-medium text-text-secondary sm:block sm:text-caption">Gunakan untuk setoran, penjemputan, dan verifikasi di layanan resmi.</p>
                    </div>
                    ...
                    <div class="flex min-w-0 flex-col items-center justify-center border-l border-border/80 pl-3 sm:pl-5">
                        <div class="w-full rounded-lg border border-border bg-surface p-1 shadow-xs sm:p-2">
                            ...
                            @else
                                <div class="flex aspect-square items-center justify-center bg-disabled-bg p-2 text-center text-[0.6875rem] text-text-secondary sm:text-caption">QR belum aktif</div>
                            @endif
                        </div>
                        <p class="mt-1.5 hidden text-center text-[0.6875rem] font-semibold leading-tight text-text-secondary sm:block sm:text-caption">Pindai untuk verifikasi</p>
                    </div>
                </div>

                <div class="relative flex items-center justify-between gap-2 border-t border-border/80 bg-warm-canvas/90 px-3 py-1 text-[0.6875rem] text-text-secondary sm:px-6 sm:py-2 sm:text-caption">
