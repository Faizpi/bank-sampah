...
async function loginBackoffice(page, email = 'admin@example.test') {
...
await run('Dashboard Warga, Petugas, Bendahara, dan Admin dapat diakses', async () => {
    ...
    await loginBackoffice(adminPage, 'superadmin@example.test');
...
await run('Warga mengajukan pickup dengan foto', async () => {
    ...
    await settle(page);
    await page.locator('textarea[name="address"]').fill('Jl. UAT E2E No. 1');
    await settle(page);
...
await run('Warga mengajukan pencairan dan hold saldo', async () => {
    ...
    await settle(page);
    await page.locator('textarea[name="pickupLocation"]').fill('Loket UAT Bank Sampah Digital');
    await settle(page);
...
await run('Admin menyetujui koreksi setoran melalui browser', async () => {
    const adminContext = await browser.newContext();
    const admin = await adminContext.newPage();
    await loginBackoffice(admin, 'superadmin@example.test');
