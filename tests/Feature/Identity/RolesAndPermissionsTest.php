...
final class RolesAndPermissionsTest extends TestCase
{
    ...
    private const PERMISSIONS = [
        ...
        'customer.create-assisted', 'customer.qr.rotate', 'customer.update', 'customer.view', 'deposit.approve', 'deposit.create',
        ...
        'grocery.view', 'ledger.adjust', 'ledger.view', 'media.retention.execute', 'mobile-service.manage', 'mobile-service.operate',
        'mobile-service.view', 'notification.view', 'pickup.cancel', 'pickup.complete',
        'pickup.execute', 'pickup.request', 'pickup.review', 'pickup.schedule', 'pickup.view', 'price.manage',
        'price.view', 'profile.update', 'profile.view', 'qr-verification.rotate', 'reconciliation.approve', 'reconciliation.create',
        'reconciliation.view', 'region.manage', 'region.view', 'report.export', 'report.view',
    ...
    private const ROLE_PERMISSIONS = [
        ...
        'admin' => ['profile.view', 'profile.update', 'backoffice.access', 'user.view', 'user.view.all', 'user.create', 'user.update', 'user.activate', 'user.verify', 'user.reject', 'user.reset-password', 'role.view', 'session.revoke', 'customer.view', 'customer.create-assisted', 'customer.update', 'customer.card.issue', 'customer.qr.rotate', 'region.view', 'region.manage', 'waste.view', 'waste.manage', 'price.view', 'price.manage', 'deposit.view', 'deposit.create', 'deposit.update-draft', 'deposit.finalize', 'ledger.view', 'correction.view-customer', 'pickup.view', 'pickup.request', 'pickup.review', 'pickup.schedule', 'pickup.execute', 'pickup.complete', 'pickup.cancel', 'withdrawal.request', 'withdrawal.view', 'withdrawal.approve', 'withdrawal.pay', 'withdrawal.cancel', 'grocery.package.view', 'grocery.package.manage', 'grocery.view', 'grocery.approve', 'grocery.prepare', 'grocery.handover', 'grocery.cancel', 'notification.view', 'announcement.view', 'announcement.manage', 'announcement.publish', 'mobile-service.view', 'mobile-service.manage', 'mobile-service.operate', 'target.view', 'target.manage', 'target.publish', 'statistics.internal.view', 'statistics.public.manage', 'qr-verification.rotate', 'report.view', 'report.export', 'audit.view'],
        'superadmin' => ['profile.view', 'profile.update', 'backoffice.access', 'user.view', 'user.view.all', 'user.create', 'user.update', 'user.activate', 'user.verify', 'user.reject', 'user.reset-password', 'role.view', 'role.manage', 'session.revoke', 'customer.view', 'customer.create-assisted', 'customer.update', 'customer.card.issue', 'customer.qr.rotate', 'region.view', 'region.manage', 'waste.view', 'waste.manage', 'price.view', 'price.manage', 'deposit.view', 'deposit.create', 'deposit.update-draft', 'deposit.finalize', 'deposit.approve', 'ledger.view', 'transaction.correct', 'transaction.reverse', 'ledger.adjust', 'correction.view-customer', 'pickup.view', 'pickup.request', 'pickup.review', 'pickup.schedule', 'pickup.execute', 'pickup.complete', 'pickup.cancel', 'withdrawal.request', 'withdrawal.view', 'withdrawal.approve', 'withdrawal.pay', 'withdrawal.cancel', 'grocery.package.view', 'grocery.package.manage', 'grocery.view', 'grocery.approve', 'grocery.prepare', 'grocery.handover', 'grocery.cancel', 'notification.view', 'announcement.view', 'announcement.manage', 'announcement.publish', 'mobile-service.view', 'mobile-service.manage', 'mobile-service.operate', 'target.view', 'target.manage', 'target.publish', 'statistics.internal.view', 'statistics.public.manage', 'qr-verification.rotate', 'report.view', 'report.export', 'audit.view', 'reconciliation.view', 'reconciliation.create', 'reconciliation.approve', 'system.settings.manage', 'system.maintenance', 'backup.run', 'backup.view', 'backup.restore', 'audit.retention.execute', 'media.retention.execute'],
    ];
    ...
    public function test_superadmin_permissions_are_exactly_the_admin_union_with_technical_and_reconciliation_permissions(): void
    {
        $this->seed(RolesAndPermissionsSeeder::class);
        ...
        $reconciliationPermissions = ['deposit.approve', 'transaction.correct', 'transaction.reverse', 'ledger.adjust'];
        $expected = collect([...$adminPermissions, ...$technicalPermissions, ...$reconciliationPermissions])->unique()->sort()->values()->all();
        $actual = Role::query()->where('name', 'superadmin')->sole()->permissions()->pluck('name')->sort()->values()->all();

        self::assertSame($expected, $actual);
        self::assertTrue(Role::query()->where('name', 'superadmin')->sole()->permissions()->whereIn('name', $reconciliationPermissions)->count() === 4);
    }
